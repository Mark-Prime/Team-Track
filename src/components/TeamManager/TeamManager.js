...
class TeamManager extends Component {
  state = {
    ...
    newLink: this.props.team[0].team_page || "",
  };
  ...
  changeMemberState = (value, target, id) => {
    ...
    if (target === "class") {
      this.props.dispatch({
        type: "SET_MEMBER_CLASS",
        payload: { value, id, target: this.props.team[0].trueid },
      });
    } else if (target === "main") {
      this.props.dispatch({
        type: "SET_MEMBER_MAIN",
        payload: { value, id, target: this.props.team[0].trueid },
      });
    }
  };
  ...
  promoteMember = (id, is_user) => {
    this.props.dispatch({
      ...
      payload: { id, team: this.props.team[0].trueid },
    });
    if (is_user) {
  ...
  deactivateTeam = () => {
    this.props.dispatch({
      ...
      payload: this.props.team[0].trueid,
    });
  };
  ...
  render() {
    return (
      <>
        ...
        <Divider orientation="center">Team League Info</Divider>
        <Row>
          <Col span={1}></Col>
          <Col span={22}>
            <Search
              value={this.state.newLink}
              enterButton="Save Link"
              size="large"
              onChange={(event) => this.changeLinkState(event)}
              onSearch={(value) => this.props.saveLink(value)}
            />
          </Col>
          <Col span={1}></Col>
        </Row>
                  ...
                  render: (text, record) => (
                    <>
                      <Select
                        ...
                        onChange={(value) =>
                          this.changeMemberState(value, "class", record.user_id)
                        }
                      >
                  ...
                  render: (text, record) => (
                    <>
                      <Select
                        defaultValue={text}
                        onChange={(value) =>
                          this.changeMemberState(value, "main", record.user_id)
                        }
                      >
