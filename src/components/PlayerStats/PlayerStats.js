...
class PlayerStats extends Component {
  ...
  parseClasses = (obj, index, stat_start, stat_end) => {
    ...
    for (const class_name of classes) {
      if (Number(index[stat_start + class_name + stat_end])) {
        obj[class_name] =
          obj[class_name] + Number(index[stat_start + class_name + stat_end]) ||
  ...
  parseResponse = () => {
    ...
    for (let index of this.props.log) {
      ...
      playerClassKills = this.parseClasses(
        playerClassKills,
        index,
        "kills_as_",
        ""
      );
      playerClassDeaths = this.parseClasses(
        playerClassDeaths,
        index,
        "deaths_as_",
        ""
      ...
      gamemode_stats[index.title].playerClassKills = this.parseClasses(
        gamemode_stats[index.title].playerClassKills || {},
        index,
        "kills_as_",
        ""
      );
      gamemode_stats[index.title].playerClassDeaths = this.parseClasses(
        gamemode_stats[index.title].playerClassDeaths || {},
        index,
        "deaths_as_",
        ""
      );
      ...
      if (!rows[rows.length - 1] || rows[rows.length - 1].date !== index.date) {
        ...
        gamemode_stats[index.title].classKills = this.parseClasses(
          gamemode_stats[index.title].classKills || {},
          index,
          "",
          ""
        );
        gamemode_stats[index.title].classDeaths = this.parseClasses(
          gamemode_stats[index.title].classDeaths || {},
          index,
          "",
          "_deaths"
        );
        ...
        <div className="statistics-container">
          ...
          <div className="stat">
            <Statistic
              title="Most Played"
              value={
                this.state.favorite_class === "heavyweapons"
                  ? "Heavy"
                  : this.state.favorite_class.charAt(0).toUpperCase() +
                    this.state.favorite_class.slice(1)
              }
              className="stat"
            />
          </div>
            ...
            <Collapse defaultActiveKey={["0"]}>
              <Panel header="All Gamemodes" key="0">
                <PlayerLineGraph
                  ...
                  player_id={this.props.player[0].id}
                />
              ...
              {Object.keys(this.state.gamemode_stats).map((gamemode) => {
                return (
                  <Panel
                    header={gamemode}
                    key={this.state.gamemode_stats[gamemode].id}
                  >
                    <PlayerLineGraph
                      title1="Damage"
                      title2="Damage Taken"
                      ...
                      dataMax={this.parseDataMax(
                        this.state.gamemode_stats[gamemode].rows
                      )}
                    />
                    ...
                    <PlayerBarGraph
                      ...
                      datakey2="deaths"
                      data={
                        this.state.gamemode_stats[gamemode]
                          .playerClassKillsArray
                      }
                    />
